...
def download_video(csv, name, video_id, start_time, duration_time):
    """
    ...
    if start_time != start_time and duration_time != duration_time:
        ...
        try:
            #print(f'ffmpeg -hide_banner -loglevel error -ss {start_time} -i "{original_video}" -to {duration_time} -c copy "{output_file}"')
            os.system(
...
for idx, row in tqdm(df_links.iterrows(), total=df_links.shape[0]):
    download_video(df_links, *row)
    df_links.to_csv('data_mine.csv', index=False)
